import './viewNurseryOneThirdTerm.css';
...
import { selectNurseryOneThirdResults, STORE_NURSERYONETHIRDRESULT } from '../../../redux/slice/nurseryOneThirdSlice';



const ViewNurseryOneThirdTerm = () => {
  
  
  const {data, isLoading} = useFetchCollection("20242025NoneThirdTerm")
  console.log(data);
...
const deleteNurseryOneSecondTermResult = async(id) => {
  try {
    await deleteDoc(doc(db, "20242025NoneThirdTerm", id));
  ...
  return (
    ...
    <div className="table">
    <h2>All Nursery One Third Term Result</h2>
         ...
         {nOneResults.map((nOneResult, index) => {
          ...
          return (
            ...
            <tr key={id}>
               ...
               <td>
                  <Link to={`/add-nurseryonethirdterm/${id}`}>
...
export default ViewNurseryOneThirdTerm;
